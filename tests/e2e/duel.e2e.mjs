...
import { makeChecker } from './lib.mjs';
...
try {
  ...
  // ---- two sessions -------------------------------------------------------
  const alice = await seedUser(db, { name: 'DuelA', room: 'lobby' });
  const bob = await seedUser(db, { name: 'DuelB', room: 'lobby' });
  const carol = await seedUser(db, { name: 'DuelC', room: 'lobby' });
  ...
  const now = Date.now();
  const challenge = await challengeFlow(store, { id: alice.id }, { name: 'DuelB' }, now);
  ...
  check('the persisted ask names both players',
    askRow.data && askRow.data.a_name === 'DuelA' && askRow.data.b_name === 'DuelB');
  ...
  const accept = await acceptFlow(store, { id: bob.id }, { from: 'DuelA' }, Date.now());
  ...
  check('each session is told its own opponent',
    frameA.opponent === 'DuelB' && frameB.opponent === 'DuelA');
  ...
  const forgedInsert = await sessionA.from('duels').insert({
    a_user: alice.id, b_user: carol.id, a_name: 'DuelA', b_name: 'DuelC',
  ...
  // ---- decline ------------------------------------------------------------
  console.log('decline');
  const c2 = await challengeFlow(store, { id: alice.id }, { name: 'DuelB' }, Date.now());
  check('a fresh challenge lands after the cancel', c2.body.ok === true);
  const declined = await declineFlow(store, { id: bob.id }, { from: 'DuelA' });
  ...
  check('the challenger is told who declined', back?.payload.name === 'DuelB');
  ...
  const dave = await seedUser(db, { name: 'DuelD', room: 'lobby' });
  ...
  const blocked = await challengeFlow(store, { id: carol.id }, { name: 'DuelA' }, Date.now());
  console.log(`  challenge busy ->  ${JSON.stringify(blocked.body)}`);
  check('a doubly-duelling player still reads as busy',
    blocked.body.ok === false && blocked.body.reason === 'DuelA is already duelling');
