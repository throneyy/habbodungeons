...
/** Is this port bindable right now? `undefined` means "no preference". */
...
export async function startStack({ pgPort, apiPort, migrations = [] } = {}) {
  ...
  // Requested ports are a PREFERENCE, not a requirement. A caller passes a
  // per-worktree pair (duel.e2e.mjs, via portFor) so the common case is
  // predictable, but nothing here persists across runs, so a port that happens
  // to be busy — a neighbouring worktree mid-run, a stray server — must not
  // fail the suite the way it would for a browser suite pinned to an origin.
  const [autoPg, autoApi] = await freePorts(2);
  pgPort = (await portFree(pgPort)) ? pgPort : autoPg;
  apiPort = (await portFree(apiPort)) ? apiPort : autoApi;
  if (pgPort === apiPort) apiPort = autoApi === pgPort ? autoPg : autoApi;
