...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Fetching server players for server:", serverId);

    // Get server with all players
    const { data: server, error: serverError } = await supabase
      .from('servers')
      .select('*, server_players(user_id)')
      .eq('id', serverId)
      .maybeSingle();
    ...
    // Get all player profiles with stats
    const userIds = server.server_players.map((sp: any) => sp.user_id);
    ...
    console.log("Fetched", members.length, "server players");
