...
serve(async (req) => {
  ...
  try {
    ...
    if (serverId) {
      ...
      if (serverBattle) {
        ...
        if (soloBattle) {
          ...
          await supabase.from("battle_states").update({ server_id: serverId }).eq("id", soloBattle.id);
    ...
    if (battle.server_id) {
      ...
      const { data: allStats } = await supabaseAdmin.from("player_stats").select("*").in("user_id", userIds);

      const { data: allProfiles } = await supabaseAdmin.from("profiles").select("*").in("id", userIds);
      ...
      const turnOrder = players.sort((a, b) => b.spd - a.spd).map((p) => p.userId);
    ...
    if (!battle.dungeons) {
      console.error("Dungeon missing for battle:", {
        battleId: battle.id,
        dungeonId: battle.dungeon_id,
        partyId: battle.party_id,
      });

      // Clean up the invalid battle state
      await supabase.from("battle_states").update({ is_active: false }).eq("id", battle.id);
    ...
    console.log(
      `Room index check: requested=${requestedRoomIndex}, max=${maxRoomIndex}, actual=${actualRoomIndex}, total rooms=${dungeonData.rooms.length}`,
    );
    ...
    if (requestedRoomIndex > maxRoomIndex) {
      ...
      await supabase.from("battle_states").update({ current_room_index: actualRoomIndex }).eq("id", battle.id);
    }
    ...
    const enemySlots = hasValidEnemy
      ? [
          { id: "E1", x: 2, y: 2 }, // Far back, against the rear wall
        ]
      : [];
